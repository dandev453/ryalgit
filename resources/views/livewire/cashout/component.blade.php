    ...
    <div class="row content-header ">
        <div class="col-xs-12 col-md-4">
            <div class="input-group">
                ...
                <input type="date" wire:model.lazy="fromDate" id="basicFlatpickr" class="form-control flatpickr pull-right active">
                @error('fromDate') <span class="text-danger">{{ $message }}</span> @enderror
                <div class="input-group-addon">
                    <i class="fa fa-calendar"></i>
                </div>
                <input type="date" wire:model.lazy="toDate" id="dateTimeFlatpickr" class="form-control flatpickr pull-right active">
            ...
            <div class="col-md-4 col-sm-12 col-xs-12 ">
                <div class="form-group">
                    <button  wire:click.prevent="Consultar()" type="button" class="btn btn-flat btn-md">Consultar</button>
                    @if($total > 0)
                    <button wire:click.prevent="Print()" type="button" class="btn btn-flat btn-md"><span><i class="fa fa-print class="text-light></i> </span>Imprimir</button>
                    @endif
                </div>
            </div>
            @endif
            ...
            <div class="col-md-4 col-sm-12 col-xs-12 float-right">
                <div class="input-group">
                    ...
                    @error('userid')<span class="text-danger"><i class="fa fa-warning-symbol"></i> {{$message}}</span>@enderror
                </div>
            </div>
                        ...
                        <table class="table table-condensed table-hover table-striped">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center"><span class="label label-success">{{ number_format($row->total,2) }}</span></td>
                                    <td class="text-center"> {{ $row->items }} </td>
                                    <td class="text-center"> {{ $row->created_at }} </td>
                                    <td class="text-center" >
                                        <button wire:click.prevent="viewDetails({{$row}})" class="btn btn-flat"><span><i class="fa fa-list"></i></span></button>
                                    </td>
                                    @endforeach
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span>  <b> ventas totales: </b> <h5><b>$ </b>{{ number_format($total,2)}}</h5></span>
